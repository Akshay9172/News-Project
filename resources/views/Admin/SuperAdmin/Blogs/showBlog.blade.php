...
@section('content')
    <div class="col-12">
        <div class="card">
                        ...
                        @foreach ($blogs as $key => $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-success" data-toggle="modal" data-target="#viewBlogModal" data-title="{{ $item->title }}" data-description="{{ $item->description }}" data-img="{{ $item->img }}">View Blog</button>
            ...
            {{-- <a href="{{ route('display-blog') }}" class="btn btn-secondary">View All Blogs</a> --}}
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <h1 id="blogTitle" class="heading-text text-2xl" style="text-decoration: underline; font-size: 1rem; line-height: 1.5; margin-top: 0.625rem;"></h1>
                    <p id="blogDescription" class="para-text" style="font-size: 0.75rem; font-weight: 400; line-height: 1.8;"></p>
                </div>
...
{{-- <script>
    $('#viewBlogModal').on('show.bs.modal', function (event) {
        ...
        var modal = $(this);
        modal.find('.modal-title').text('Blog Details');
        modal.find('#blogTitle').text(title);
        modal.find('#blogDescription').text(description);
...
{{-- <script>
    $('#viewBlogModal').on('show.bs.modal', function (event) {
        ...
        var blogId = button.data('id'); // Get blog ID
        ...
        $.get('/blog/' + blogId, function(data) {
            ...
            modal.find('#blogDescription').text(data.description);
            modal.find('#blogImg').attr('src', data.img);
        });
    });
</script> --}}
...
<script>
    $('#viewBlogModal').on('show.bs.modal', function (event) {
        ...
        $.get('/blog/' + blogId, function(data) {
            modal.find('.modal-title').text(data.title);
            modal.find('#blogDescription').text(data.description);
            modal.find('#blogImg').attr('src', '{{ asset('storage/') }}' + '/' + data.img);
        });
    });
</script>
